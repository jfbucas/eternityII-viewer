
puzzles[ nb_puzzles ] = "brendan/pieces_10x04.txt.js,Brendan 10x04".split( splitreg ); nb_puzzles++;

if ( puzzle == "brendan/pieces_10x04.txt.js" ) {

board_w = 10;
board_h = 4;


pieces[ p ] = "0,0,2,1".split( splitreg ); p++;
pieces[ p ] = "0,0,2,3".split( splitreg ); p++;
pieces[ p ] = "0,0,3,2".split( splitreg ); p++;
pieces[ p ] = "0,0,3,3".split( splitreg ); p++;
pieces[ p ] = "0,1,4,2".split( splitreg ); p++;
pieces[ p ] = "0,1,4,3".split( splitreg ); p++;
pieces[ p ] = "0,1,5,2".split( splitreg ); p++;
pieces[ p ] = "0,1,5,3".split( splitreg ); p++;
pieces[ p ] = "0,1,6,1".split( splitreg ); p++;
pieces[ p ] = "0,1,6,3".split( splitreg ); p++;
pieces[ p ] = "0,1,7,3".split( splitreg ); p++;
pieces[ p ] = "0,1,8,1".split( splitreg ); p++;
pieces[ p ] = "0,2,4,2".split( splitreg ); p++;
pieces[ p ] = "0,2,5,1".split( splitreg ); p++;
pieces[ p ] = "0,2,6,2".split( splitreg ); p++;
pieces[ p ] = "0,2,7,1".split( splitreg ); p++;
pieces[ p ] = "0,2,7,3".split( splitreg ); p++;
pieces[ p ] = "0,2,8,1".split( splitreg ); p++;
pieces[ p ] = "0,3,4,2".split( splitreg ); p++;
pieces[ p ] = "0,3,4,3".split( splitreg ); p++;
pieces[ p ] = "0,3,5,2".split( splitreg ); p++;
pieces[ p ] = "0,3,7,1".split( splitreg ); p++;
pieces[ p ] = "0,3,7,2".split( splitreg ); p++;
pieces[ p ] = "0,3,8,1".split( splitreg ); p++;
pieces[ p ] = "4,4,5,7".split( splitreg ); p++;
pieces[ p ] = "4,4,6,7".split( splitreg ); p++;
pieces[ p ] = "4,4,8,5".split( splitreg ); p++;
pieces[ p ] = "4,5,4,7".split( splitreg ); p++;
pieces[ p ] = "4,6,8,7".split( splitreg ); p++;
pieces[ p ] = "4,7,5,5".split( splitreg ); p++;
pieces[ p ] = "4,7,6,6".split( splitreg ); p++;
pieces[ p ] = "4,7,8,6".split( splitreg ); p++;
pieces[ p ] = "4,8,8,8".split( splitreg ); p++;
pieces[ p ] = "5,5,6,8".split( splitreg ); p++;
pieces[ p ] = "5,5,8,6".split( splitreg ); p++;
pieces[ p ] = "5,6,5,8".split( splitreg ); p++;
pieces[ p ] = "5,6,8,6".split( splitreg ); p++;
pieces[ p ] = "5,6,8,7".split( splitreg ); p++;
pieces[ p ] = "5,7,6,7".split( splitreg ); p++;
pieces[ p ] = "6,8,7,8".split( splitreg ); p++;

// piece,space,rotation
fixed[ f ] = "0,0,1".split( splitreg ); f++;

nb_pieces = p;
nb_fixed = f;

}

