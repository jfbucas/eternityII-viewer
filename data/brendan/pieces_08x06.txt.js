
puzzles[ nb_puzzles ] = "brendan/pieces_08x06.txt.js,Brendan 08x06".split( splitreg ); nb_puzzles++;

if ( puzzle == "brendan/pieces_08x06.txt.js" ) {

board_w = 8;
board_h = 6;


pieces[ p ] = "0,0,1,2".split( splitreg ); p++;
pieces[ p ] = "0,0,2,1".split( splitreg ); p++;
pieces[ p ] = "0,0,3,1".split( splitreg ); p++;
pieces[ p ] = "0,0,3,3".split( splitreg ); p++;
pieces[ p ] = "0,1,4,3".split( splitreg ); p++;
pieces[ p ] = "0,1,5,2".split( splitreg ); p++;
pieces[ p ] = "0,1,5,3".split( splitreg ); p++;
pieces[ p ] = "0,1,6,3".split( splitreg ); p++;
pieces[ p ] = "0,1,7,3".split( splitreg ); p++;
pieces[ p ] = "0,1,8,2".split( splitreg ); p++;
pieces[ p ] = "0,1,9,3".split( splitreg ); p++;
pieces[ p ] = "0,2,4,3".split( splitreg ); p++;
pieces[ p ] = "0,2,6,1".split( splitreg ); p++;
pieces[ p ] = "0,2,6,2".split( splitreg ); p++;
pieces[ p ] = "0,2,6,3".split( splitreg ); p++;
pieces[ p ] = "0,2,7,1".split( splitreg ); p++;
pieces[ p ] = "0,2,7,2".split( splitreg ); p++;
pieces[ p ] = "0,2,9,1".split( splitreg ); p++;
pieces[ p ] = "0,3,4,2".split( splitreg ); p++;
pieces[ p ] = "0,3,5,1".split( splitreg ); p++;
pieces[ p ] = "0,3,5,2".split( splitreg ); p++;
pieces[ p ] = "0,3,6,1".split( splitreg ); p++;
pieces[ p ] = "0,3,7,2".split( splitreg ); p++;
pieces[ p ] = "0,3,8,1".split( splitreg ); p++;
pieces[ p ] = "4,4,4,6".split( splitreg ); p++;
pieces[ p ] = "4,4,4,9".split( splitreg ); p++;
pieces[ p ] = "4,5,4,6".split( splitreg ); p++;
pieces[ p ] = "4,5,8,6".split( splitreg ); p++;
pieces[ p ] = "4,6,8,7".split( splitreg ); p++;
pieces[ p ] = "4,7,4,9".split( splitreg ); p++;
pieces[ p ] = "4,7,9,8".split( splitreg ); p++;
pieces[ p ] = "4,7,9,9".split( splitreg ); p++;
pieces[ p ] = "4,8,7,5".split( splitreg ); p++;
pieces[ p ] = "4,8,7,8".split( splitreg ); p++;
pieces[ p ] = "4,9,6,5".split( splitreg ); p++;
pieces[ p ] = "5,5,5,8".split( splitreg ); p++;
pieces[ p ] = "5,5,5,9".split( splitreg ); p++;
pieces[ p ] = "5,5,6,7".split( splitreg ); p++;
pieces[ p ] = "5,6,8,7".split( splitreg ); p++;
pieces[ p ] = "5,7,6,9".split( splitreg ); p++;
pieces[ p ] = "5,9,8,8".split( splitreg ); p++;
pieces[ p ] = "5,9,9,9".split( splitreg ); p++;
pieces[ p ] = "6,6,6,9".split( splitreg ); p++;
pieces[ p ] = "6,6,8,8".split( splitreg ); p++;
pieces[ p ] = "6,8,7,9".split( splitreg ); p++;
pieces[ p ] = "6,8,9,7".split( splitreg ); p++;
pieces[ p ] = "7,7,7,8".split( splitreg ); p++;
pieces[ p ] = "7,8,7,9".split( splitreg ); p++;

// piece,space,rotation
fixed[ f ] = "0,0,1".split( splitreg ); f++;

nb_pieces = p;
nb_fixed = f;

}

